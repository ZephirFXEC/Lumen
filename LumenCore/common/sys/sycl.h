...
namespace embree {
    ...
    template<typename T>
    __forceinline T sub_group_broadcast(T x, sycl::id<1> local_id) {
        return SYCL_SUBGROUP::group_broadcast<SYCL_EXT_ONEAPI::sub_group>(this_sub_group(), x, local_id);
    }

    template<typename T>
    __forceinline T sub_group_make_uniform(T x) {
