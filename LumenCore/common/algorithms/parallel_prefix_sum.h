...
namespace embree {
    ...
    __forceinline Value
    parallel_prefix_sum(ParallelPrefixSumState<Value> &state, Index first, Index last, Index minStepSize,
    ...
    __forceinline Value
    parallel_prefix_sum(const SrcArray &src, DstArray &dst, size_t N, const Value &identity, const Add &add,
                        const size_t SINGLE_THREAD_THRESHOLD = 4096) {
        ...
        if (N < SINGLE_THREAD_THRESHOLD) {
            ...
            parallel_prefix_sum(state, size_t(0), size_t(N), size_t(1024), identity,
                                [&](const range<size_t> &r, const Value &sum) -> Value {
            ...
            return parallel_prefix_sum(state, size_t(0), size_t(N), size_t(1024), identity,
                                       [&](const range<size_t> &r, const Value &sum) -> Value {
