...
namespace embree {
    ...
    public:
        ...
        __forceinline parallel_partition_task(T *array,
                ...
                : array(array), N(N), is_left(is_left), reduction_t(reduction_t), reduction_v(reduction_v),
                  identity(identity),
                  numTasks(min((N + BLOCK_SIZE - 1) / BLOCK_SIZE, min(TaskScheduler::threadCount(), MAX_TASKS))) {}

        __forceinline const range<ssize_t> *
        findStartRange(size_t &index, const range<ssize_t> *const r, const size_t numRanges) {
            size_t i = 0;
        ...
        __forceinline void swapItemsInMisplacedRanges(const size_t numLeftMisplacedRanges,
            ...
            const range<ssize_t> *r_range = findStartRange(rightLocalIndex, rightMisplacedRanges,
                                                           numRightMisplacedRanges);
        ...
        __forceinline size_t partition(V &leftReduction, V &rightReduction) {
            ...
            parallel_for(numTasks, [&](const size_t taskID) {
                ...
                const size_t mid = serial_partitioning(array, startID, endID, local_left, local_right, is_left,
                                                       reduction_t);
    ...
    __noinline size_t parallel_partitioning(T *array,
        ...
        if (unlikely(end - begin < BLOCK_SIZE))
            ...
            std::unique_ptr<partition_task> p(
                    new partition_task(&array[begin], end - begin, identity, is_left, reduction_t, reduction_v,
                                       BLOCK_SIZE));
