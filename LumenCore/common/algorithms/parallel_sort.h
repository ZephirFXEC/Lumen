    ...
    public:
        ...
        void sort(const size_t blockSize) {
            ...
            if (N <= blockSize) // handles also special case of 0!
            {
                ...
                const size_t numThreads = min((N + blockSize - 1) / blockSize, TaskScheduler::threadCount(),
                                              size_t(MAX_TASKS));
...
#pragma nounroll
        ...
        void tbbRadixIteration(const Key shift, const bool last,
            ...
            parallel_for_affinity(numTasks,
                                  [&](size_t taskIndex) { tbbRadixIteration0(shift, src, dst, taskIndex, numTasks); },
                                  ap);
            parallel_for_affinity(numTasks,
                                  [&](size_t taskIndex) { tbbRadixIteration1(shift, src, dst, taskIndex, numTasks); },
                                  ap);
        }
