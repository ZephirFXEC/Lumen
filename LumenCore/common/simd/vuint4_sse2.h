...
namespace embree {
    ...
    struct vuint<4> {
        ...
        static __forceinline vuint4 load(const vbool4 &mask, const void *a) {
            return _mm_and_si128(_mm_load_si128((__m128i *) a), mask);
        }

        static __forceinline vuint4 loadu(const vbool4 &mask, const void *a) {
            return _mm_and_si128(_mm_loadu_si128((__m128i *) a), mask);
        }

        static __forceinline void store(const vboolf4 &mask, void *ptr, const vuint4 &i) {
            store(ptr, select(mask, i, load(ptr)));
        }

        static __forceinline void storeu(const vboolf4 &mask, void *ptr, const vuint4 &i) {
            storeu(ptr, select(mask, i, loadu(ptr)));
        }
...
/*#if defined(__SSE4_1__)
  ...
  __forceinline vuint4 max(unsigned int  a, const vuint4& b) { return max(vuint4(a),b); }*/
    ...
    template<>
    __forceinline unsigned int extract<0>(const vuint4 &b) { return _mm_cvtsi128_si32(b); }
