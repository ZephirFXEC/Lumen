...
namespace embree {
...
#if defined(__AVX2__)
        const Vec3fa h_n = _mm_fnmadd_ps(a.m128, r.m128,
                                         vfloat4(1.0));  // First, compute 1 - a * r (which will be very close to 0)
    ...
    struct __aligned(16) Vec3fx {
        ...
        __forceinline Vec3fx(const float x, const float y, const float z, const float w) : m128(
                _mm_set_ps(w, z, y, x)) {}
