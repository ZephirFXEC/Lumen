...
auto Sphere::CalculateBounds(AABB &outbox) const -> AABB
{
    outbox = { m_Center - glm::vec3(m_Radius), m_Center + glm::vec3(m_Radius) };
    return outbox;
}

auto Sphere::DeepCopy() const -> std::shared_ptr<IHittable> { return std::make_shared<Sphere>(*this); }
...
auto Plane::CalculateBounds(AABB &outbox) const -> AABB
{
    outbox = { m_Center - glm::vec3(0.0001F), m_Center + glm::vec3(0.0001F) };
    return outbox;
}

auto Plane::DeepCopy() const -> std::shared_ptr<IHittable> { return std::make_shared<Plane>(*this); }
